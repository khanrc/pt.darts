...
def draw_bounding_boxes(image, boxes, img_path, labels=None, colors=None, fill=False, width=1, font=None, font_size=10):
    ...
    with torch.no_grad():
        if not isinstance(image, torch.Tensor):
            ...
            # new_image = tf.Normalize(mean=(0,)*3, std=(255,)*3)(image)
            # new_image = tf.Compose([lambda x: tf.Normalize(mean=x.mean(), std=x.std()/255)(x)])(image)
            pil_image = tf.ToPILImage()(image)
            ndarr = np.array(pil_image)
