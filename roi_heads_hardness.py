...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    Returns:
        ...
        box_loss (Tensor)
    """
    labels_copy = labels # len==batch size len(labels[0]) = 512 (whatever was used
    # at initialization of roi head batch size per image)
    ...
    return classification_loss, box_loss, hardness
...
class RoIHeads(torch.nn.Module):
    ...
    def forward(self,
        ...
        # raise AttributeError(features['0'].shape, len(proposals))
        box_features = self.box_roi_pool(features, proposals, image_shapes)
        # raise AttributeError(box_features.shape)
        ...
        if self.training:
            ...
            loss_classifier, loss_box_reg, hardness = fastrcnn_loss(
