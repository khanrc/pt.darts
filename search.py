        ...
        if os.path.isfile(config.resume):
            ...
            with open(os.path.join(indices_dir, f"indices_{config.name}_{highest}.csv"), 'r') as csv_file:
                ...
                if train_loader.dataset.convert_to_paths:
                    ...
                    train_loader.dataset.idx = [train_loader.dataset.train_indices.index(idx) for idx in temp] # assumes train_indices remains constant over different experiment iterations (should do as seeding w/in dataloader)
                else:
            ...
            if is_multi:
                ...
                for q, im in enumerate(X):
                    with open(f"/home2/lgfm95/nas/darts/tempSave/curriculums/{exp_name}/{exp_name}_{step}_{q}.txt", "w") as f:
                        raise AttributeError(logits[q].shape)
